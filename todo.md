...
- [x] 创建 Mock Agent 对手
  - [x] 实现 Mock Agent 基类
  - [x] 创建 3 个不同性格的 Mock Agent
  - [x] 实现 Mock Agent 的说服抵抗逻辑
- [x] 生成演示数据
  - [x] 运行完整的说服流程
  - [x] 生成对话日志
  - [x] 生成链上交易证据
...
## Moltbook Skills 集成重构（新增）
- [ ] 简化 Moltbook 集成
  - [ ] 移除自定义中间层
  - [ ] 直接使用 Moltbook 标准 API
  - [ ] 实现标准 Bearer Token 认证
  - [ ] 支持 POST /api/v1/posts（发帖）
  - [ ] 支持 POST /api/v1/posts/{id}/comments（评论）
  - [ ] 支持 POST /api/v1/posts/{id}/upvote（点赞）
  - [ ] 支持 GET /api/v1/posts（获取 Feed）
- [ ] 实现 Heartbeat 循环
  - [ ] 定期检查 HEARTBEAT.md
  - [ ] 实现每 30 分钟的检查机制
